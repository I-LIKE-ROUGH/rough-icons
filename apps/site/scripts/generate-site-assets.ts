...
for (const name of Object.keys(staticIconImports).sort()) {
  ...
  const paths = icon.paths
    .map(
      (path) =>
        `<path d="${path.d}" fill="${path.fill}" stroke="${path.stroke}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>`,
    )
    .join('');
  await writeFile(
    resolve(iconRoot, `${name}.svg`),
  ...
  records.push({
    ...
    tags,
    searchText:
      `${name} ${iconAliases.join(' ')} ${tags.join(' ')}`.toLowerCase(),
  });
}
