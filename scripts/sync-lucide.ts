...
const version = execFileSync('npm', ['view', `@lucide/icons@${requested}`, 'version'], { encoding: 'utf8' }).trim().split('\n').at(-1)!;
...
for (const packageName of ['core', 'icons', 'react']) {
  const publicPackagePath = resolve(root, `packages/${packageName}/package.json`);
