...
import type { RuntimeRoughTablerIconProps } from './types.js';
...
function generate(source: NormalizedIconSource, options: RoughIconOptions) {
  const key = `${source.iconSet}:${source.iconStyle}:${source.name}:${JSON.stringify(options)}`;
...
export function createRuntimeIcon(source: NormalizedIconSource) {
  const Icon = forwardRef<SVGSVGElement, RuntimeRoughTablerIconProps>(
    function RuntimeRoughTablerIcon(
