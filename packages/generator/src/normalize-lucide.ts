...
function assertAttributes(
  ...
  const unknown = Object.keys(attributes).filter(
    (key) => key !== 'key' && !allowed.includes(key),
  );
  if (unknown.length)
    throw new Error(`Unsupported ${tag} attributes: ${unknown.join(', ')}`);
}

function normalizeNode(node: LucideIconNode): NormalizedIconNode {
  if (node.length !== 2)
    throw new Error(`Unsupported nested SVG element: ${node[0]}`);
...
export function normalizeLucideIcon(
  icon: LucideIconData,
): NormalizedIconSource {
  try {
    ...
    throw new Error(
      `Failed to normalize ${icon.name}: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
