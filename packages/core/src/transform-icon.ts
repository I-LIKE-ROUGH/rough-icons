...
import type { GeneratedIconPath, NormalizedIconNode, NormalizedIconSource, RoughIconOptions } from './types.js';

const generator = new RoughGenerator();

function roundedRectanglePath(node: Extract<NormalizedIconNode, { type: 'rectangle' }>): string {
  const radius = Math.min(node.rx ?? node.ry ?? 0, node.width / 2, node.height / 2);
  if (!radius) return `M${node.x} ${node.y}h${node.width}v${node.height}h-${node.width}Z`;
...
function drawNode(node: NormalizedIconNode, options: Options): Drawable {
  const nodeOptions = node.type === 'circle' && node.fill ? { ...options, fill: node.fill, fillStyle: 'solid' } : options;
  switch (node.type) {
    case 'path': return generator.path(svgpath(node.d).toString(), nodeOptions);
    case 'line': return generator.line(node.x1, node.y1, node.x2, node.y2, nodeOptions);
    case 'polyline': return generator.linearPath(node.points.map(([x, y]) => [x, y]), nodeOptions);
    case 'polygon': return generator.polygon(node.points.map(([x, y]) => [x, y]), nodeOptions);
    case 'circle': return generator.circle(node.cx, node.cy, node.r * 2, nodeOptions);
    case 'ellipse': return generator.ellipse(node.cx, node.cy, node.rx * 2, node.ry * 2, nodeOptions);
    case 'rectangle': return generator.path(roundedRectanglePath(node), nodeOptions);
  }
}
...
export function transformIcon(icon: NormalizedIconSource, input: RoughIconOptions = {}) {
  ...
  return { name: icon.name, width: icon.width, height: icon.height, paths: generatedPaths };
}
